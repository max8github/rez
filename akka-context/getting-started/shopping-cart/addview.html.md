...
In the following code, we add support for **[JWT](../../sdk/auth-with-jwts.html)** -based bearer tokens to the HTTP endpoint with just a single line. While not shown here, you can define all kinds of rules based on the claims supplied in a token.
...
```
Extracting the user ID from context is quite easy. Modify the `get` function so that it rejects attempts to query a shopping cart that does not belong to the caller.
...
| `/carts/my` | `GET` | Retrieves the currently active shopping cart for the current user, or `404` if the cart has not been created |
...
With JWT authentication in place, it is now slightly more difficult to invoke the service via `curl`, but only because we have to generate a valid token. Since this sample does not validate for specific issuers, any valid token will be fine. You can create your own tokens on [JWT.io](https://jwt.io/), or you can use the one from the following `curl` example, which interrogates the user’s current shopping cart.
...
```
While `curl` is useful for automation and testing, sometimes it might be easier to use a tool to generate requests for your endpoints. With the Akka local console running, you can navigate to the HTTP endpoint and then build a request, as shown in the following screenshot:

![Shopping cart request builder](../_images/shoppingcart_request_builder.png)
...
![Shopping cart request trace view](../_images/shoppingcart_request_flow.png)
...
<!-- <footer> -->
<!-- <nav> -->
[A simple shopping cart service](build-and-deploy-shopping-cart.html) [Samples and blueprints](../samples.html)
<!-- </nav> -->
