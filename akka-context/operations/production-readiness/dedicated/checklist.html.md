...
What Akka expects of customer applications before going live in Dedicated, the deployment model where Akka Automated Operations runs inside a cloud account Akka owns and operates. The platform handles a substantial portion of the operational surface, and in Dedicated that surface includes the cloud account itself, so the customer has less to run than in any other model; but a robust production system is still a shared outcome, and the items below sit on the customer side of that line.
...
In Dedicated, Akka owns and operates the cloud account, so the account, networking, and cloud-security items that a BYOC customer would own are Akka’s, and the items below focus on the applications you build.

Walk this page before going live, using the at-a-glance below as the quick entry point and the detailed checklists for the specifics. The items are a baseline drawn from what Akka has found necessary across Dedicated deployments; each application should add anything specific to its own request flows and failure modes. Modifications or removals are also possible but should be discussed with your Forward Deployed Engineer (FDE), the named Akka engineer assigned to your account, so the original rationale can be shared and the change evaluated jointly.
...
Akka operates the full stack beneath your applications: the cloud account itself, the in-region network, account security monitoring, the runtime, region installation, scaling, patching, platform observability, backups, and the operational rituals around them. The items on this page are the customer-side preparation that complements that work, so production is reliable from day one. Your FDE walks through the principal items ahead of go-live.
...
|  | **Akka’s side** Akka runs a parallel internal pre-production validation against the platform-side commitments captured in the RACI: backups, replication, observability stack, baseline regional alerts, certificate monitoring, cloud-account audit logging and security monitoring, and similar. This page covers the customer-side complement; the two are designed to pair. |
...
| Akka | FDE walks the sizing exercise, validates platform-cores and service-cores numbers, names the AZ (Availability Zone) spread and persistence headroom recommendations. Owns and sizes the cloud account and network in Dedicated. |
...
- Confirm with your FDE that Akka has sized the platform, account, and network for your forecast: node count and instance types, 3+ AZ spread, VPC / CIDR (Classless Inter-Domain Routing) and security groups, container registry, and persistence headroom with auto-scaling ceilings
...
- External secret stores reachable from prod with IAM (Identity and Access Management)
- LLM rate limits cover peak; fallback model configured
- Upstream/downstream dependencies mapped with SLAs (Service Level Agreements)
- Cross-service single points of failure documented (shared brokers, DBs, identity)
- Per-service configs reviewed for prod parity (ACLs (Access Control Lists), JWT issuers, secrets, autoscaling)
...
- Exporter configured (e.g., OTLP (OpenTelemetry Protocol), Prometheus, Splunk HEC (HTTP Event Collector), Azure Monitor, Google Cloud, or any OpenTelemetry-compatible backend) pointing at your enterprise platform
...
- Per-service RED (Rate, Errors, Duration) dashboards at p50/p95/p99
...
- Image scan blocks on critical CVEs (Common Vulnerabilities and Exposures) (Trivy, Snyk, or equivalent)
- SAST (Static Application Security Testing) and SCA (Software Composition Analysis) gates; findings triaged
...
- Customer-managed TLS (BYO domains, BYO certs): customer monitors expiration, rotation procedure rehearsed pre-go-live
...
Not every customer is regulated, but where you are, these are the items auditors and regulators ask for. Consider each; skip only with a documented reason.

| Role | Responsibility |
| --- | --- |
| Customer | Ingests and retains audit evidence in your own SIEM, owns continuity planning for the dependencies your application relies on, and rehearses producing compliance evidence for your regulators. |
| Shared | Akka delivers control-plane and cloud-account audit events and maintains platform compliance posture (SOC 2 / SOC 3, penetration-test summary, subprocessor list via trust.akka.io) that feeds your evidence. |
...
|  | **Availability level and replication mode** Availability comes in levels; choose per service against its needs.
  ...
  - **Single region, multiple availability zones**: the baseline, resilient to the loss of an availability zone. On Akka-operated infrastructure, high availability across zones is the default.
...
|  | This page is part of the Dedicated Shared Responsibility Model and is maintained as a living reference, updated periodically as Akka’s platform and operational practices evolve. To receive notifications of material updates, email [support@akka.io](mailto:support@akka.io). |

<!-- <footer> -->
<!-- <nav> -->
[RACI](raci.html) [Shared responsibility model](../byoc/shared-responsibility.html)
<!-- </nav> -->
