...
file, facility Stripe-onboarding completeness) and a post-fulfillment payment-scheduling path via a new
Consumer reacting to `Fulfilled`. Rescue refund and the waiting list (Phases 2/3 of the source design
...
**Scale/Scope**: 4 new components (`PaymentEntity`, `PlayerPaymentProfileEntity`, `SlotPaymentView`, `StripeService`), 1 new endpoint (`StripeWebhookEndpoint`), 2 new Consumers/TimedActions (`PaymentSchedulingAction`, `CommitmentCutoffTimedAction` — plus a resolution-point counterpart), 5 touched existing files (`FacilityState`/`FacilityEntity`/`FacilityEvent`, `ResourceState`/`ResourceEvent`, `ReservationState`, `CourtBookingWorkflow`, `FacilityEndpoint`/`ResourceEndpoint`)
...
- **II. Design Principles**: No violation. Domain records (`PaymentState`, `PricingPolicy`,
  ...
  rather than growing the latter (research.md #5). Descriptive naming throughout — no generic
  `Event`/`Service`/`Manager` names.
...
├── orchestration/
│   └── CourtBookingWorkflow.java               # MODIFIED — book() gains FR-005/FR-012 gates before reservationGateway.submit() (research.md #9); return type changes from bare ReservationHandle to a new sealed BookingHandle
├── customer/facility/
...
doc's own Code Mapping suggestion), touching five existing packages at their established extension
...
(`ReservationHandle` → a new sealed `BookingHandle`) is the one call-site ripple that reaches into
`BookingTools`/`BookingEndpoint` — both already sit downstream of `book()` today and need only a
`switch` over the new result type, not a redesign.
