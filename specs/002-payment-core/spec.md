...
- **FR-005**: `CourtBookingWorkflow` MUST check `PlayerPaymentProfile` for the requester's resolved `userId` before submitting a booking. A player with a saved payment method already on file MUST see no different conversational behavior. A player with none MUST NOT have a reservation submitted on their behalf — `CourtBookingWorkflow` MUST instead surface a Stripe-hosted card-collection link (Checkout in setup mode, or a Payment Link) and require them to retry booking once it's completed.
...
- **FR-012**: `CourtBookingWorkflow` MUST reject a booking at submission time — not merely at the commitment cutoff — if that facility has a `PricingPolicy` configured but no completed Stripe connected-account onboarding, mirroring FR-005's booking-time gate on the player side. A price with nowhere to route funds MUST NOT be allowed to reach a payable reservation state.
...
- **SC-007**: 0% of booking attempts by a player without a completed card-on-file (or a facility without completed Stripe onboarding) ever result in a locked court — both gates apply at booking time, per FR-005/FR-009/FR-012.
