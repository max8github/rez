...
unchanged. No observable behavior has changed yet — every new field is always empty until Phase 3.
...
- [ ] T012 [P] [US1] Add unit test case(s) to `ReservationEntityTest.java` asserting `init()` with a
...
- [ ] T013 [P] [US1] Create `TelegramEndpointIntegrationTest.java` covering: first-contact resolution
...
- [ ] T014 [US1] Inject `HttpClientProvider` into `TelegramEndpoint`'s constructor and construct an
      `IdentityClient` from it, in
      `reservation/reservation/src/main/java/com/rezhub/reservation/api/TelegramEndpoint.java`
- [ ] T015 [US1] In `TelegramEndpoint.onUpdate`, when `senderExternalId` is non-blank, call
      `identityClient.resolveOrCreate("TELEGRAM", senderExternalId, Optional.empty())` and pass the
      result into the new `OriginRequestContext` `identityUserId` field, in
...
identity is resolved and durably persisted on their reservation. This is also, by construction, the
MVP: everything else in this feature verifies properties of what this phase already built.
...
- [ ] T016 [US2] Add a case to `TelegramEndpointIntegrationTest.java` pointing identity resolution at
      ...
      `reservation/reservation/src/test/java/com/rezhub/reservation/api/TelegramEndpointIntegrationTest.java`
      (same file as T013 — sequential, not parallel)

### Implementation for User Story 2

- [ ] T017 [US2] Guard `TelegramEndpoint.onUpdate` to skip the identity resolution call entirely when
...
- [ ] T018 [US3] Add a test asserting that a separate, later `ReservationEntity::getReservation` query
...
- [ ] T019 [P] Reconcile `hit-backend/docs/cross-product-identity.md`'s Open Items and Stage 3
...
- [ ] T020 [P] Reconcile `rez/docs/payments-cancellation-waitlist-design.md`'s `PlayerPaymentProfile`
...
- [ ] T021 Run `mvn -o verify` from `reservation/reservation/` to confirm all new and existing tests
      pass together
- [ ] T022 Execute `quickstart.md`'s manual verification steps end-to-end against a locally running
      stack, per `specs/001-telegram-identity-resolution/quickstart.md`
- [ ] T023 Update `spec.md`'s Status field from `Draft` to a closed/complete status once T021 and T022
...
- **Polish (Final Phase)**: Depends on Phases 3–5 being complete — the doc reconciliation
  (T019/T020) needs the real, implemented shape to describe accurately
...
- T012 and T013 (different test files)
- T019 and T020 (different repos entirely)
...
1. Complete Phase 1 (Setup) and Phase 2 (Foundational — required, not optional, unlike a typical
   feature's foundational phase; nothing compiles without it here)
2. Complete Phase 3 (User Story 1)
3. **STOP and VALIDATE**: run T012/T013, confirm both pass
...
— Phase 2 is a hard sequential gate, and Phases 4/5 are thin verification passes over Phase 3's work,
not separately substantial implementation efforts. One implementer working through phases 1→5→Polish
in order is the natural shape here.
...
- Commit after each phase (Setup, Foundational, US1, US2, US3, Polish) — matches this project's
  established discipline of committing after each logical phase rather than one giant diff
