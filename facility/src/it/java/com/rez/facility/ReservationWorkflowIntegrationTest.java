...
public class ReservationWorkflowIntegrationTest extends KalixIntegrationTestKitSupport {
  ...
  public void shouldReserve() {
    ...
    String reservationId1 = issueNewReservationRequest(resourceIds);
    System.out.println("reservationId1 = " + reservationId1);
    assertBookedAtResource(reservationId1, resourceIds, 0);
    assertReservationState(reservationId1, FULFILLED);

    String reservationId2 = issueNewReservationRequest(resourceIds);
    System.out.println("reservationId2 = " + reservationId2);
    assertBookedAtResource(reservationId2, resourceIds, 1);
    assertReservationState(reservationId2, FULFILLED);

    String reservationId3 = issueNewReservationRequest(resourceIds);
    System.out.println("reservationId3 = " + reservationId3);
    assertBookedAtResource(reservationId3, resourceIds, -1);
   ...
   */
  private void assertBookedAtResource(String reservationId, List<String> resIds, int index) {
    await()
      ...
      .untilAsserted(() -> {
        ...
        if(index >= 0 && index < resIds.size()) {
          ...
          assertThat(resource.timeWindow()[12]).as("resource %s should be booked with %s",
  ...
  @NotNull
  private String issueNewReservationRequest(List<String> resourceIds) {
  ...
  private ReservationState getWorkflowState(String reservationId) {
    return webClient.get().uri("/rez/" + reservationId)
      .retrieve()
