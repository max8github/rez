...
public class ReservationEntity extends Workflow<ReservationState> {
    ...
    public WorkflowDef<ReservationState> definition() {
        ...
        Step fulfill = step("fulfill").asyncCall(Fulfill.class, cmd -> {
          ...
          return componentClient.forAction().call(DelegatingServiceAction::book).params(fulfilled)
              .execute()
              .thenCompose(req -> timers().cancel(timerName(cmd.reservationId)))
              .thenApply(done -> "Ok");
        }).andThen(String.class, cmd -> effects().updateState(currentState().withState(FULFILLED)).transitionTo("obsolete"));

        Step obsolete = step("obsolete")
          .asyncCall(() -> {
              log.info("step obsolete");
              String rezId = currentState().reservationId();
              return timers().startSingleTimer(
                "reservation-to-obsolete-timer-" + rezId,
        ...
        Step complete = step("complete")
          .asyncCall(String.class, (reservationId) -> {
              log.info("step complete");
              return timers().cancel(timerName(reservationId));
          })
          .andThen(Done.class, __ -> effects()
        ...
        Step cancel = step("cancel").call(String.class, rezId -> {
            ...
            Set<String> resourceIds = currentState().resources();
        ...
        }).andThen(String.class, cmd -> effects().updateState(currentState().withState(CANCELLED)).end());
...
//                .timeout(ofSeconds(5))
    ...
    public Effect<String> complete() {
        return effects().transitionTo("complete", currentState().reservationId()).thenReply("reservation completed.");
    }
    ...
    public Effect<String> expire()  {
        log.info("Reservation {} is forced to timeout (no avail resources)", currentState().reservationId());
    ...
    public record Init(String facilityId, Reservation reservation, java.util.Set<String> resources) {}
